...
def pattern_recognition(df, method, lag=14):
    ...
    for i in range(period+lag, len(df)):
        ...
        # tenemos que asegurarnos de que no haya posiciones largas y que el precio
        # de cierre no sea el punto máximo del horizonte de búsqueda de patrones
        if (df['cumsum'][i]==0) and (df['Close'][i]!=max(df['Close'][i-period:i])):
            ...
            # Obtenga el nodo principal j con el valor máximo en el período de búsqueda de
            # patrones. Tenga en cuenta que el marco de datos está en el índice de fecha y hora.
            # Queremos convertir el resultado de idxmax en un número de índice numérico.
            ...
            # si el nodo principal j es significativamente más grande 
            # que el nodo i, pasaríamos a la siguiente frase
            ...
            # we would move on to the next phrase
            # tratamos de encontrar el nodo k entre el nodo j y el nodo i
            # si el nodo k no es significativamente diferente del nodo i,
            # pasaríamos a la siguiente frase
            k=0
            if moveon==True:
                moveon=False
                for k in range(j, i):    
            ...
            # we would move on to the next phrase
            # tratamos de encontrar el nodo l entre el nodo j y el final del horizonte de búsqueda
            # de patrones, tenga en cuenta que comenzamos desde el nodo j hacia la izquierda porque
            # necesitamos encontrar otro nodo inferior m más tarde, que comenzaría desde la izquierda
            # comenzando de esta manera podemos asegurarnos de que encontraría un nodo de hombro n 
            # entre el nodo m y el nodo l si el nodo l no es significativamente diferente del nodo i,
            # pasaríamos a la siguiente frase
            l=0
            if moveon==True:
                moveon=False
            ...
            # we would move on to the next phrase
            # tratamos de encontrar el nodo m entre el nodo l y el final del horizonte de
            # búsqueda de patrones esta vez comenzamos de izquierda a derecha como de
            # costumbre si el nodo m no es significativamente diferente del nodo i
            # pasaríamos a la siguiente frase
            m=0
            if moveon==True:
                moveon=False        
            ...
            # we would move on to the next phrase
            # obtenga el nodo de hombro n con el valor máximo entre el nodo m y el nodo l
            # tenga en cuenta que el marco de datos está en el índice de fecha y hora queremos
            # convertir el resultado de idxmax en un número de índice numérico si el nodo n
            # es significativamente más grande que el nodo i y significativamente más pequeño
            # que el nodo j lo haríamos pasar a la siguiente frase
            n=0
            if moveon==True:
                moveon=False        
            ...
            # tratamos de encontrar el nodo de hombro o entre el nodo k y el nodo i si el
            # nodo o no es significativamente diferente del nodo n configuraríamos las 
            # señales y las coordenadas para la visualización también necesitamos actualizar
            # cumsum y entry_rsi para salir de la nota comercial que moveon sigue siendo
            # establecer como Verdadero ayudaría al algoritmo a ignorar esta ronda de iteración
        ...
        # cada vez que tenemos una posición de retención, el contador aumentaría constantemente
        # si se cumple cualquiera de las condiciones de salida. Salimos de la operación con una
        # posición larga y actualizamos counter, entry_rsi y cumsum. Puede que se pregunte
        # por qué necesitamos cumsum. bueno, esto es para mantener posiciones en caso de que
        # quiera verificar el rendimiento de la carteraS
        if entry_rsi!=0 and moveon==False:
            counter+=1
            if (df['rsi'][i]-entry_rsi>exit_rsi) or (counter>exit_days):
                df.at[df.index[i],'signals']=1
                df['cumsum']=df['signals'].cumsum()
                counter=0
                entry_rsi=0
